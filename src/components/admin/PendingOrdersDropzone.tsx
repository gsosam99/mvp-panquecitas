...
import type { PendingOrdersParseResult } from "@/types";

interface PendingOrdersDropzoneProps {
  onCommitSuccess: (count: number, unmatched: string[]) => void;
}

type UploadState = "idle" | "parsing" | "previewing" | "uploading" | "done";

export function PendingOrdersDropzone({ onCommitSuccess }: PendingOrdersDropzoneProps) {
  const [state, setState] = useState<UploadState>("idle");
  const [dragOver, setDragOver] = useState(false);
  const [parseResult, setParseResult] = useState<PendingOrdersParseResult | null>(null);
  ...
  const parseFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      const { parsePendingOrdersExcel } = await import("@/lib/excel-parser");
      const result = await parsePendingOrdersExcel(buffer);
  ...
  async function handleCommit() {
    ...
    try {
      ...
      if (data.unmatched_sap_codes?.length) {
        toast.warning(`${data.inserted} pedidos cargados · ${data.unmatched_sap_codes.length} códigos SAP sin PDV asociado`);
      } else {
        toast.success(`${data.inserted} pedidos pendientes cargados`);
      }
    } catch {
  ...
  if (state === "idle" || state === "parsing") {
    return (
      <div
        ...
        onClick={() => document.getElementById("pending-orders-file-input")?.click()}
      >
        <input
          id="pending-orders-file-input"
          type="file"
          accept=".xlsx,.xls"
          className="hidden"
        ...
        {state === "parsing" ? (
          ...
          <>
            ...
            <p className="font-medium text-slate-700">Arrastra el Excel de pedidos pendientes aquí</p>
            <p className="text-sm text-slate-400 mt-1">o haz clic para seleccionar (.xlsx, .xls)</p>
            <p className="text-xs text-amber-600 mt-2">
              Formato aún no confirmado por SAP — el parser busca columnas "Nº cliente", "Cantidad" y "Fecha" por
              nombre.
            </p>
          </>
        )}
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="flex flex-wrap gap-2 mt-1">
              <Badge variant="default">{parseResult.valid.length} pedidos</Badge>
        ...
        <div className="flex justify-end gap-3">
          ...
          <Button onClick={handleCommit} disabled={!parseResult.valid.length || state === "uploading"}>
            {state === "uploading" ? "Cargando…" : `Confirmar · ${parseResult.valid.length} pedidos`}
          </Button>
        </div>
      </div>
    );
